...
CREATE TABLE IF NOT EXISTS `hservicedb`.`task` (
  ...
  CONSTRAINT `project_task`
    FOREIGN KEY (`project`)
    REFERENCES `hservicedb`.`project` (`id_project`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
...
-- -----------------------------------------------------
-- Table `hservicedb`.`invited_user`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `hservicedb`.`invited_user` (
  `invited_user_id` INT NOT NULL AUTO_INCREMENT,
  `email` VARCHAR(50) NOT NULL,
